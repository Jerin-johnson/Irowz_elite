...
router.post("/message", async (req, res) => {
    try {
        ...
        if (message.toLowerCase().includes("under 1000")) {
            const products = await Product.find({ salePrice: { $lte: 1000 } }).limit(
                5
            );
            ...
            const response = products.length
                ...
                products
                    .map((p) => `🔹 ${p.productName} - ₹${p.salePrice}`)
                    .join("\n")
        ...
        if (order && message.toLowerCase().includes("last order")) {
            return res.json({
                success: true,
                response: `Your last order ${order.orderId} is being deliverd.for more details look in proflie/My orders`,
            });
        }
